...
class UserChangePassword extends CFormModel {
    ...
    public function rules() {
        return Yii::app()->controller->id == 'recovery' ? array(
            ...
            array('password, verifyPassword', 'length', 'max' => 128, 'min' => 4, 'message' => Yii::t("user", "Incorrect password (minimal length 4 symbols).")),
            ...
            array('oldPassword, password, verifyPassword', 'length', 'max' => 128, 'min' => 4, 'message' => Yii::t("user", "Incorrect password (minimal length 4 symbols).")),
            array('verifyPassword', 'compare', 'compareAttribute' => 'password', 'message' => Yii::t("user", "Retype Password is incorrect.")),
    ...
    public function verifyOldPassword($attribute, $params) {
        if (User::model()->notsafe()->findByPk(Yii::app()->user->id)->password != Yii::app()->getModule('user')->encrypting($this->$attribute))
            $this->addError($attribute, Yii::t("user", "Old Password is incorrect."));
    }
